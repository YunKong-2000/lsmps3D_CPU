...
- [o] 设计 `src/provisional/provisional.*` 模块接口。
- [o] 使用当前速度场计算黏性项。
- [o] 加入重力项和后续可扩展的外力项。
- [o] 计算流体粒子的临时速度 $\mathbf{u}^{*}$。
- [o] 计算壁面粒子的临时速度，满足后续 PPE 右端项中壁面速度参与散度离散的需求。
- [o] 输出临时速度、黏性加速度和外力加速度诊断字段。
- [o] 使用解析速度场或简单黏性扩散算例验证临时速度更新。

当前实现说明：

- Provisional 模块返回 `ProvisionalVelocityResult`，包含 `provisional_velocity`、`viscous_acceleration`、`body_acceleration`、`velocity_delta` 和每粒子状态。
- 黏性项使用 LSMPS `regular.inverse_moment` 计算速度各分量拉普拉斯。
- 第一版体力项包含重力和可选外部加速度。
- 对流体粒子计算黏性项、重力项和临时速度；对壁面粒子计算体力导致的临时速度，用于后续 PPE 散度项。
- 测试 `provisional_velocity_test` 输出 `output/provisional_velocity_12x12x12.vtk` 用于 ParaView 检查。
...
- [o] 串联临时速度计算、PPE 装配求解和压力修正模块。
- [o] 每个时间步更新邻域搜索、自由面识别和 LSMPS 逆矩阵。
- [o] 管理时间步、输出间隔和诊断日志。
- [o] 输出 CFL、最大速度、压力残差和自由面粒子数量。

当前实现说明：

- 时间推进模块位于 `src/time_integration/time_stepper.*`。
- `TimeStepper::advanceOneStep` 按顺序执行邻域搜索、自由面识别、LSMPS 逆矩阵构造、Provisional、PPE 和 Correction。
- 每个时间步都会根据更新后的位置重建邻域、自由面状态和 LSMPS 逆矩阵，不复用上一时刻的空间关系。
...
| 2026-06-23 | 完成约 8000 流体粒子静水算例性能基准，记录 PPE、LSMPS 和自由面识别瓶颈 | 已完成 |
| 2026-06-23 | 优化 PPE 散度计算，直接按邻域读取临时速度，去除每粒子重复构造 `ux/uy/uz` 全场数组；PPE 平均耗时下降约 27.99%，总单步耗时下降约 11.95% | 已完成 |
| 2026-06-23 | 优化 LSMPS 矩阵构造：无壁面邻居粒子直接复用 `regular` 作为 `pressure_neumann`，不改变离散结果；LSMPS 平均耗时下降约 21.30%，总单步耗时再下降约 8.64% | 已完成 |
| 2026-06-23 | 完成第二轮性能优化：PPE 按行批量装配、LSMPS 诊断开关和自由面数密度粗筛；总单步耗时进一步下降约 10.41%，相对最初基准累计下降约 27.92% | 已完成 |
| 2026-06-23 | 更新性能基准文档，记录最新静水算例输出：平均单步耗时 `15.26210 s`，PPE `6.97012 s`、LSMPS `4.54806 s`、自由面识别 `1.71165 s` | 已完成 |
| 2026-06-23 | 建立溃坝算例稳定性研究文档 `docs/development/modules/dam_break_stability_research.md`，总结前沿粒子 x 方向拉伸、邻域各向异性退化、LSMPS 失效和后续稳定化路线 | 已完成 |
| 2026-06-23 | 完成溃坝稳定性阶段一诊断增强：输出粒子间距、数密度、方向覆盖和邻域方向张量几何质量指标 | 已完成 |
| 2026-06-24 | 撤回溃坝稳定性阶段二时间步与速度修正保护：保留原始 CFL 时间步约束，移除单步绝对位移限制和压力修正速度限幅；原因是该方案对 `t ≈ 0.2 s` 附近崩溃无明显改善 | 已完成 |
| 2026-06-24 | 完成 Particle Shifting 算法设计文档 `docs/development/modules/particle_shifting_algorithm_design.md`，明确拉伸修复、最小距离排斥、壁面半空间约束、自由面投影和 VTK 诊断方案 | 已完成 |
...
8. 基于 `docs/development/modules/dam_break_stability_research.md` 开发溃坝稳定性控制链路；阶段一粒子间距、数密度、方向覆盖和几何质量诊断已完成。
9. 阶段二时间步与速度修正保护已试验并撤回；下一步开发受约束 particle shifting，包含数密度均匀化、最小距离排斥、壁面法向投影和自由面状态分级处理。
10. 再增加 LSMPS/PPE/Correction 坏粒子保护，避免局部坏粒子触发全局崩溃。
11. 根据 `docs/development/modules/performance_baseline_hydrostatic_8000.md` 继续性能优化；当前下一步性能方向是 PETSc 求解器和预条件器参数评估。
12. 扩展输出配置模块，增加输出字段级别、VTK legacy/VTU 格式选择和重启文件管理。
