...
8. 根据 `docs/development/modules/performance_baseline_hydrostatic_8000.md` 继续性能优化；PPE divergence 重复构造、无壁面邻居压力矩阵重复构造、PPE 按行装配、LSMPS 诊断开关和自由面数密度粗筛已完成。为保持离散精度，不采用跨时间步 LSMPS 矩阵复用；为控制内存占用，暂不做邻居几何 stencil 缓存；OpenMP 并行暂不实现。下一步优先评估 PETSc 求解器和预条件器参数。
9. 增加进一步运行期稳定性控制，包括粒子越界检查、失败步诊断输出、重力/体力时间步限制和黏性时间步限制。
10. 扩展输出配置模块，增加输出字段级别、VTK legacy/VTU 格式选择和重启文件管理。
11. 基于 TimeStepper 进入基础自由面算例开发，例如三维水箱扰动或三维溃坝短程算例。
